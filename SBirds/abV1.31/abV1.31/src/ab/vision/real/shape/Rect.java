...
public class Rect extends Body
{
   ...
   protected double pwidth = -1, plength = -1;
	...
	{
		return String.format("Rect: id:%d type:%s hollow:%b Area:%d w:%7.3f h:%7.3f a:%3.3f at x:%3.1f y:%3.1f Bottomup:%.2f Topdown:%.2f penetration[0]:%.2f penetration[1]:%.2f displacement:%f support:%f downwards:%.2f", id, type, hollow, area, pwidth, plength, angle, centerX, centerY,bottomUpFactorValue,topDownFactorValue,penetrationWeight[0],penetrationWeight[1],displacementWeight,supportWeight,downwardsFactorValue);
	}
}
