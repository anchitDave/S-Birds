...
//        System.out.println("velocity " + _ux);
	...
	public int getTapTime(Rectangle sling, Point release, Point target, int tapInterval,ABObject firstBlock)
	{
		if (tapInterval == 0)
			return 0;
		...
		if(tapPoint.x>firstBlock.x){
			distance = (int)firstBlock.x-sling.x;
			
			tapInterval = 97;
			r = ((double)tapInterval/100);
			tapPoint.setLocation(new Point((int)(distance * r + sling.x) , target.y));
			return getTimeByDistance(sling, release, tapPoint);
		}
//		tapPoint.setLocation(firstBlock.getCenterX() - 30, firstBlock.getCenterY()+5);
		return getTimeByDistance(sling, release, tapPoint);
		
	}
	
	public int getNaiveTapTime(Rectangle sling, Point release, Point target, int tapInterval){
		if (tapInterval == 0)
			return 0;
		Point tapPoint = new Point();
		int distance = target.x - sling.x;
